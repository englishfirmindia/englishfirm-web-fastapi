...
def _make_question(question_id: int):
    ...
    q.module = "reading"
    q.question_type = "reading_fib"
    q.difficulty_level = 2
    q.time_limit_seconds = 120
    q.content_json = {"passage": "The ___ is bright."}
    q.evaluation = MagicMock()
    q.evaluation.evaluation_json = {
        "correctAnswers": {"1": "sun"},
        "scoringRules": {"marksPerBlank": 1, "isCaseSensitive": False, "trimWhitespace": True},
    }
    return q


@patch("routers.reading.fill_in_blanks.start_session")
def test_start_returns_session(mock_start, client):
    mock_start.return_value = {
        "session_id": "fib1",
        "total_questions": 1,
        "questions": [{"question_id": 1, "module": "reading", "question_type": "reading_fib",
                        "difficulty_level": 2, "time_limit_seconds": 120, "content_json": {}}],
    }
    response = client.post("/api/v1/questions/reading/fill-in-blanks/start", json={})
    assert response.status_code == 200
    assert response.json()["session_id"] == "fib1"


@patch("routers.reading.fill_in_blanks.get_scorer")
@patch("routers.reading.fill_in_blanks.mark_submitted")
@patch("routers.reading.fill_in_blanks.get_session")
def test_submit_correct(mock_get_session, mock_mark, mock_get_scorer, client):
    ...
    mock_get_scorer.return_value.score.return_value = ScoringResult(
        ...
        breakdown={"hits": 1, "total": 1, "blank_results": {"1": True}}
    )

    response = client.post("/api/v1/questions/reading/fill-in-blanks/submit", json={
        "session_id": "fib1", "question_id": 1, "user_answers": {"1": "sun"}
    })
    assert response.status_code == 200
    data = response.json()
...
def test_submit_question_not_found(mock_get_session, client):
    ...
    response = client.post("/api/v1/questions/reading/fill-in-blanks/submit", json={
        "session_id": "fib1", "question_id": 999, "user_answers": {"1": "sun"}
    })
