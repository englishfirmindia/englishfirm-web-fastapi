...
from db.models import User, QuestionFromApeuni, QuestionExplanation
...
def list_questions(
    ...
    if practiced is not None:
        practiced_subq = practiced_questions_subq(db, current_user.id, "reading_drag_and_drop")
        if practiced:
    ...
    practiced_ids = practiced_question_ids_in(db, current_user.id, "reading_drag_and_drop", page_qids)
...
def submit(
    ...
    try:
        ...
        log.error(
            "[Reading FIB] scoring failed q=%d sid=%s err=%s: %s",
    ...
    # Reading FIB uses flat dict {blank_id: correct_value}
