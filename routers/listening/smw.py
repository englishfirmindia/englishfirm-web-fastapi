...
router = APIRouter(prefix="/listening/smw", tags=["Listening - Select Missing Word"])
...
def list_questions(
    ...
    query = db.query(QuestionFromApeuni).filter(
        ...
        QuestionFromApeuni.question_type == "listening_smw",
    )
...
def submit(
    ...
    scorer = get_scorer("listening_smw")
    try:
        result = scorer.score(
        ...
        log.error(
            "[Listening SMW] scoring failed q=%s sid=%s err=%s: %s",
    ...
    persist_answer_to_db(
        session=session, question_id=question_id, question_type="listening_smw",
