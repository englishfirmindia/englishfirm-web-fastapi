...
def list_questions(
    ...
    query = db.query(QuestionFromApeuni).filter(
        ...
        QuestionFromApeuni.question_type == "listening_mcq_multiple",
    )
    ...
    if practiced is not None:
        practiced_subq = practiced_questions_subq(db, current_user.id, "listening_mcq_multiple")
        if practiced:
    ...
    practiced_ids = practiced_question_ids_in(db, current_user.id, "listening_mcq_multiple", page_qids)
...
def submit(
    ...
    selected_options = req.resolved_options()
    ...
    try:
        ...
        log.error(
            "[Listening MCM] scoring failed q=%s sid=%s err=%s: %s",
