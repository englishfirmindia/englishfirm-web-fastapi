...
router = APIRouter(prefix="/listening/hcs", tags=["Listening - Highlight Correct Summary"])
...
def list_questions(
    ...
    query = db.query(QuestionFromApeuni).filter(
        ...
        QuestionFromApeuni.question_type == "listening_hcs",
    )
    ...
    if practiced is not None:
        practiced_subq = practiced_questions_subq(db, current_user.id, "listening_hcs")
        if practiced:
    ...
    practiced_ids = practiced_question_ids_in(db, current_user.id, "listening_hcs", page_qids)
...
def submit(
    ...
    scorer = get_scorer("listening_hcs")
    try:
        result = scorer.score(
        ...
        log.error(
            "[Listening HCS] scoring failed q=%s sid=%s err=%s: %s",
    ...
    persist_answer_to_db(
        session=session, question_id=question_id, question_type="listening_hcs",
