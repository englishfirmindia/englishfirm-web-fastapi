...
router = APIRouter(prefix="/listening/fib", tags=["Listening - Fill in the Blanks"])
...
def list_questions(
    ...
    query = db.query(QuestionFromApeuni).filter(
        ...
        QuestionFromApeuni.question_type == "listening_fib",
    )
    ...
    if practiced is not None:
        practiced_subq = practiced_questions_subq(db, current_user.id, "listening_fib")
        if practiced:
    ...
    practiced_ids = practiced_question_ids_in(db, current_user.id, "listening_fib", page_qids)
...
def submit(
    ...
    try:
        ...
        log.error(
            "[Listening FIB] scoring failed q=%s sid=%s err=%s: %s",
    ...
    persist_answer_to_db(
        session=session, question_id=question_id, question_type="listening_fib",
