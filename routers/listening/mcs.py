...
router = APIRouter(prefix="/listening/mcs", tags=["Listening - MCS"])
...
def list_questions(
    ...
    query = db.query(QuestionFromApeuni).filter(
        ...
        QuestionFromApeuni.question_type == "listening_mcq_single",
    )
    ...
    if practiced is not None:
        practiced_subq = practiced_questions_subq(db, current_user.id, "listening_mcq_single")
        if practiced:
    ...
    practiced_ids = practiced_question_ids_in(db, current_user.id, "listening_mcq_single", page_qids)
...
def submit(
    ...
    scorer = get_scorer("listening_mcs")
    try:
        result = scorer.score(
        ...
        log.error(
            "[Listening MCS] scoring failed q=%s sid=%s err=%s: %s",
