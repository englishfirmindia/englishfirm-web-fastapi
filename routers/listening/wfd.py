...
def list_questions(
    ...
    query = db.query(QuestionFromApeuni).filter(
        ...
        QuestionFromApeuni.question_type == "listening_wfd",
    )
    ...
    if practiced is not None:
        practiced_subq = practiced_questions_subq(db, current_user.id, "listening_wfd")
        if practiced:
    ...
    practiced_ids = practiced_question_ids_in(db, current_user.id, "listening_wfd", page_qids)
