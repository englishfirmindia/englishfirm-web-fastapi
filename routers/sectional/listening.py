...
def start_exam(
    ...
    # Free tier: lifetime flag. Paid tier: sectionals_per_month counter.
...
def in_progress(
    ...
    """List in-progress listening sectionals the user can resume, sourced
    ...
    return {"pending": fetch_pending_sectionals(db, current_user.id, "listening")}
...
def finish_exam(
    ...
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
    # Gating happens at exam START — see enforce_free_sectional_or_paid.
):
    session_id = payload["session_id"]
