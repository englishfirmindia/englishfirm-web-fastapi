...
def _kick_off_azure(task_type: str, question_id: int, audio_url: str, user_id: int,
                    expected_answers: list = None, extra_warnings: list = None) -> None:
...
def list_questions(
    ...
    query = db.query(QuestionFromApeuni).filter(
        ...
        QuestionFromApeuni.question_type == "answer_short_question",
    )
    ...
    if practiced is not None:
        practiced_subq = practiced_questions_subq(db, current_user.id, "answer_short_question")
        if practiced:
    ...
    practiced_ids = practiced_question_ids_in(db, current_user.id, "answer_short_question", page_qids)
...
def submit(
    ...
    if not expected_answers:
        logger.error(
            "[ASQ submit] q=%d expected_answers missing in evaluation_json — scoring with warning",
            question_id,
        )
    ...
    scorer.score(
        ...
        answer={
            ...
            "kick_off_fn": lambda t, q, u: _kick_off_azure(
                t, q, u, current_user.id, expected_answers, extra_warnings),
        },
    )
    mark_submitted(session_id, question_id, 0)
    persist_speaking_answer_pending(session, question_id, "answer_short_question", audio_url)
