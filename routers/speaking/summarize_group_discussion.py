...
def list_questions(
    ...
    query = db.query(QuestionFromApeuni).filter(
        ...
        QuestionFromApeuni.question_type == "summarize_group_discussion",
    )
    ...
    if practiced is not None:
        practiced_subq = practiced_questions_subq(db, current_user.id, "summarize_group_discussion")
        if practiced:
    ...
    practiced_ids = practiced_question_ids_in(db, current_user.id, "summarize_group_discussion", page_qids)
...
def submit(
    ...
    if not key_points and not stimulus_audio_url:
        logger.error(
            "[SGD submit] q=%d both keyPoints and audio_url missing — scoring with warning",
            question_id,
        )
