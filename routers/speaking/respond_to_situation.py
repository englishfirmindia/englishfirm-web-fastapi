...
def list_questions(
    ...
    if practiced is not None:
        practiced_subq = practiced_questions_subq(db, current_user.id, "ptea_respond_situation")
        if practiced:
    ...
    practiced_ids = practiced_question_ids_in(db, current_user.id, "ptea_respond_situation", page_qids)
...
def submit(
    ...
    if not key_points and not stimulus_audio_url:
        logger.error(
            "[RTS submit] q=%d both keyPoints and audio_url missing — scoring with warning",
            question_id,
        )
