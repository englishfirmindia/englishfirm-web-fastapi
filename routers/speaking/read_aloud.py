...
def list_questions(
    ...
    if practiced is not None:
        practiced_subq = practiced_questions_subq(db, current_user.id, "read_aloud")
        if practiced:
    ...
    # `sort=recent` orders practice-mode-practiced questions first (most
    # recent at the top) then never-practiced rows in id order. Sectional
    # and mock attempts deliberately do NOT bubble a question up — see
    # services/question_list_helper.py for the rationale.
    if sort == 'recent':
    ...
    practiced_ids = practiced_question_ids_in(db, current_user.id, "read_aloud", page_qids)
