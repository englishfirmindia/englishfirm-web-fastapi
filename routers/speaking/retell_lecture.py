...
def list_questions(
    ...
    query = db.query(QuestionFromApeuni).filter(
        ...
        QuestionFromApeuni.question_type == "retell_lecture",
    )
    ...
    if practiced is not None:
        practiced_subq = practiced_questions_subq(db, current_user.id, "retell_lecture")
        if practiced:
    ...
    practiced_ids = practiced_question_ids_in(db, current_user.id, "retell_lecture", page_qids)
...
def get_upload_url(
    ...
    key = f"recordings/{current_user.id}/retell_lecture/{question_id}/{uuid.uuid4()}.aac"
...
def submit(
    ...
    persist_speaking_answer_pending(session, question_id, "retell_lecture", audio_url)
